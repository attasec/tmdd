...
- 🟢 **`T001`** YAML template injection via init command name/description `[T:Tampering]`
  - `M001` — Escape or quote user-provided name/description values before substituting into YAML templates during init, using yaml.dump() or proper YAML string quoting to prevent structural injection (`src/commands/init.py`:36-39) *(default)*
- 🟢 **`T006`** Arbitrary file creation via init command path argument `[T:Tampering]`
  - `M007` — Validate and normalize the target path in init command, ensuring it resolves to a subdirectory of the current working directory or an explicitly allowed location (`src/commands/init.py`:22-23) *(default)*
- 🟡 **`T007`** Information disclosure via generated output files `[I:Info Disclosure]`
  - `M008` — Include .tmdd/out/ in the default .gitignore generated by tmdd init, and add a warning comment in generated output files noting they contain sensitive security analysis (`src/commands/init.py`) *(default)*
...
- 🟢 **`T005`** Denial of service via deeply nested or oversized YAML files `[D:Denial of Service]`
  - `M006` — Add file size checks in load_yaml() before parsing, rejecting YAML files exceeding a reasonable threshold (e.g., 1MB) to prevent resource exhaustion (`src/utils.py`:101-117) *(default)*
- 🟡 **`T008`** No audit trail for threat model modifications `[R:Repudiation]`
  - **Risk accepted**
...
- 🟡 **`T009`** Prompt injection via threat model content in generated AI prompts `[T:Tampering]`
  - `M010` — Add clear delimiter markers and role boundaries in generated AI prompts to reduce the effectiveness of prompt injection from threat model content (`src/generators/agent_prompt.py`:17-19, `src/generators/threat_prompt.py`) *(default)*
- 🟡 **`T007`** Information disclosure via generated output files `[I:Info Disclosure]`
  - `M008` — Include .tmdd/out/ in the default .gitignore generated by tmdd init, and add a warning comment in generated output files noting they contain sensitive security analysis (`src/commands/init.py`) *(default)*
...
- 🟠 **`T002`** Supply chain attack via CDN JavaScript dependencies `[T:Tampering]`
  - `M002` — Pin CDN library versions with Subresource Integrity (SRI) hashes on all <script> tags in generated HTML to prevent tampered JavaScript from executing (`report.py`:943-945, `diagram.py`:143-145) *(default)*
  - `M003` — Bundle Cytoscape.js and dagre libraries locally instead of relying on CDN, eliminating the external dependency and enabling fully offline report viewing (`report.py`:943-945) *(default)*
- 🟡 **`T003`** Stored XSS via innerHTML in diagram JavaScript `[T:Tampering]`
  - `M004` — Replace innerHTML assignments in _DIAGRAM_JS with textContent for plain text values, or use DOM createElement/createTextNode for safe HTML construction (`report.py`:572-872) *(default)*
  - `M005` — Implement a JavaScript HTML-escape helper function in _DIAGRAM_JS and apply it to all threat model values (names, descriptions, labels) before inserting into HTML strings (`report.py`:572-872) *(default)*
- 🟡 **`T004`** Stored XSS via innerHTML in node-info panel `[T:Tampering]`
  - `M004` — Replace innerHTML assignments in _DIAGRAM_JS with textContent for plain text values, or use DOM createElement/createTextNode for safe HTML construction (`report.py`:572-872) *(default)*
  - `M005` — Implement a JavaScript HTML-escape helper function in _DIAGRAM_JS and apply it to all threat model values (names, descriptions, labels) before inserting into HTML strings (`report.py`:572-872) *(default)*
- 🟡 **`T007`** Information disclosure via generated output files `[I:Info Disclosure]`
  - `M008` — Include .tmdd/out/ in the default .gitignore generated by tmdd init, and add a warning comment in generated output files noting they contain sensitive security analysis (`src/commands/init.py`) *(default)*
- 🟢 **`T010`** Script tag breakout in embedded JSON diagram data `[T:Tampering]`
  - `M005` — Implement a JavaScript HTML-escape helper function in _DIAGRAM_JS and apply it to all threat model values (names, descriptions, labels) before inserting into HTML strings (`report.py`:572-872) *(default)*
  - `M011` — Enhance _safe_json() to escape additional dangerous characters beyond </ including HTML entities and ensure Content-Type is set correctly for embedded JSON data (`report.py`:113-114) *(default)*
- 🟢 **`T011`** Report output path traversal via --name parameter `[T:Tampering]`
  - `M012` — Validate the --name output filename in report.py by stripping path separators and applying safe_name() or os.path.basename() before joining with the output directory, consistent with diagram.py's safe_name() approach (`report.py`:1085-1088) *(default)*
